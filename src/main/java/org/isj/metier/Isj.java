...
*/
        ...
        new Isj().importerListeNote("C:\\Users\\cena\\Downloads\\cena\\test_cheihk\\notes_TEST.xlsx");

       //new Isj().setNewClasse(1, 20, 2018, "ING 1");
...
//        new Isj().saveModule("C:\\Users\\cena\\Downloads\\module licence.xlsx");
    ...
    public void createExcelNoteFile(long code_evaluation, String pathOut) throws IOException, MessagingException {
        ...
        row.createCell(1).setCellValue("oui");
    ...
    public double[] MeilleureNoteEvalution(long codeEvaluation) {
        ...
        return new double[]{noteMax, noteMin};
    }
            ...
            while (rowIt.hasNext()) {
                ...
                if (numRow > 0) {
                    ...
                    if (!libelle.isEmpty() && !codeUe.isEmpty() && !cred.isEmpty()) {
                        ...
                        if(Integer.valueOf(cred) > 0) {
                            Module module1=retrouverModule(mod);
                            Niveau niveau1=retrouverNiveau(Long.valueOf(niveau));
                            Specialite specialite1=retrouverSpecialite(specialite);
                            UE ue = new UE(libelle, "", codeUe, UE.Statut.ACTIVE, Integer.valueOf(cred), module1,niveau1,specialite1);
        ...
        while (sheetIterator.hasNext()) {
            ...
            while (rowIt.hasNext()) {
                ...
                if (numRow > 0) {
                    ...
                    anDeb = Long.valueOf(dataFormatter.formatCellValue(row.getCell(5)));
...
//                        System.out.println(module);
    ...
    public void importerTypeEvaluation(String pathOut) throws IOException, SQLException {
        ...
        while (sheetIterator.hasNext()) {
            ...
            String oldLibEns = dataFormatter.formatCellValue(sheet.getRow(4).getCell(3)).trim();
            String oldCodeUe = dataFormatter.formatCellValue(sheet.getRow(4).getCell(4)).trim();
            String tmpAnne = dataFormatter.formatCellValue(sheet.getRow(0).getCell(1)).trim();
            ...
            while (rowIterator.hasNext()) {
                ...
                if (numrow > 3) {
                    ...
                    libelle = dataFormatter.formatCellValue(row.getCell(0)).toUpperCase().trim();
                    description = dataFormatter.formatCellValue(row.getCell(1)).trim();
                    pourcentage = Float.valueOf(dataFormatter.formatCellValue(row.getCell(2)).trim());
                    libEns = dataFormatter.formatCellValue(row.getCell(3)).trim();
                    codeUe = dataFormatter.formatCellValue(row.getCell(4)).trim();

                    if(libEns.isEmpty()) libEns = oldLibEns;
                    else oldLibEns =dataFormatter.formatCellValue(row.getCell(3)).trim();
            ...
            while (sheetIterator.hasNext()) {
                ...
                for (Row row : sheet) {
                    ...
                    if (numrow > 0) {
                        ...
                        matricule = dataFormatter.formatCellValue(row.getCell(0)).trim();
                        nom = dataFormatter.formatCellValue(row.getCell(1)).trim();
                        prenom = dataFormatter.formatCellValue(row.getCell(2)).trim();
                        if (!matricule.equalsIgnoreCase("")) {
                            classe = retrouverClasse(dataFormatter.formatCellValue(row.getCell(3)).trim());

                            sexe = dataFormatter.formatCellValue(row.getCell(7)).trim();
                            ecoleOrigine = dataFormatter.formatCellValue(row.getCell(8)).trim();
                            email = dataFormatter.formatCellValue(row.getCell(9)).trim();
                            nomPere = dataFormatter.formatCellValue(row.getCell(11)).trim();
                            nomMere = dataFormatter.formatCellValue(row.getCell(13)).trim();
                            regionOrigin = dataFormatter.formatCellValue(row.getCell(15)).trim();
                            Dtype = dataFormatter.formatCellValue(row.getCell(16)).trim();

                            professionPere = dataFormatter.formatCellValue(row.getCell(12)).trim();
                            professionMere = dataFormatter.formatCellValue(row.getCell(14)).trim();

                            try {
                                dateNaiss = row.getCell(5).getDateCellValue();
                                telpEtud = Integer.valueOf(dataFormatter.formatCellValue(row.getCell(4)).trim().replaceAll(" ",""));
                                telpMere = Integer.valueOf(dataFormatter.formatCellValue(row.getCell(17)).trim().replaceAll(" ",""));
                                telpPere = Integer.valueOf(dataFormatter.formatCellValue(row.getCell(18)).trim().replaceAll(" ",""));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }

                            codeAuth = String.valueOf(new Eleve().code());
        ...
        while (sheetIterator.hasNext()) {
            ...
            while (rowIterator.hasNext()) {
                ...
                if (numrow > 3) {
                    ...
                    description = dataFormatter.formatCellValue(row.getCell(1)).toUpperCase().trim();
                    libelle = dataFormatter.formatCellValue(row.getCell(2)).trim();
                    ...
                    if (numrow >= 3) {
                        ...
                        while (cellIterator.hasNext()) {
                            ...
                            if (cell.getColumnIndex() == 0) {
                                matricule = dataFormatter.formatCellValue(cell).trim();
                                candidat = retrouverEtudiantMatricule(matricule);
                            }
                            if (cell.getColumnIndex() >= 2 && cell.getStringCellValue().equalsIgnoreCase("oui")) {
                                codeUe = dataFormatter.formatCellValue(sheet.getRow(1).getCell(cell.getColumnIndex())).toUpperCase().trim();
                                libEns = dataFormatter.formatCellValue(sheet.getRow(2).getCell(cell.getColumnIndex())).trim();
                                ...
                                if (id != -1) {
                                    EstInscrit estInscrit = new EstInscrit("", codeUe, EstInscrit.Statut.VALIDE, candidat, enseignement);
                                    estInscritFacade.create(estInscrit);
                                    //System.out.println("" + " " + matricule + " " + " " + libEns + " " + candidat+ " " + retrouverEnseignement(libEns, codeUe, anneDebut));
                                }
                            }
    ...
    public void importerListeNote(String pathOut) throws SQLException, IOException {
        ...
        int numeroTable = 0;
            ...
            while (sheetIterator.hasNext()) {
                Sheet sheet = sheetIterator.next();
                String tmpAnne = dataFormatter.formatCellValue(sheet.getRow(0).getCell(1));
                ...
                while (rowIterator.hasNext()) {
                    Row row = rowIterator.next();
                    int numrow = row.getRowNum();
                    String libelle, libelleEval, matricule = "", codeUE, typeEval, description, oldLibelle, oldCode;

                    oldLibelle = dataFormatter.formatCellValue(sheet.getRow(2).getCell(1)).trim();
                    oldCode = dataFormatter.formatCellValue(sheet.getRow(1).getCell(1)).toUpperCase().trim();
                    ...
                    if (numrow >= 4) {
                        ...
                        while (cellIterator.hasNext()) {
                            ...
                            if (cell.getColumnIndex() > 0 && !matricule.isEmpty()) {
                                libelleEval = dataFormatter.formatCellValue(sheet.getRow(2).getCell(cell.getColumnIndex())).toUpperCase().trim();
                                codeUE = dataFormatter.formatCellValue(sheet.getRow(1).getCell(cell.getColumnIndex())).toUpperCase().trim();

                                if (libelleEval.isEmpty()) {
                                    libelleEval = oldLibelle;
                                } else {oldLibelle = libelleEval;}

                                if (codeUE.isEmpty()){
                                    codeUE = oldCode;
                                } else {oldCode = codeUE;}

                                typeEval = dataFormatter.formatCellValue(sheet.getRow(3).getCell(cell.getColumnIndex())).toUpperCase().trim();
                                ...
                                try {
                                    ...
                                    System.out.println(id_typEvalation+" "+numrow);

                                    //Seules les notes valides sont enregistrées
                                    if(id_typEvalation != -1 && valeurNote!=-1 && id_estInscrit!=-1) {
                                        TypeEvaluation typeEvaluation = new TypeEvaluationFacade().find(id_typEvalation);
                                        System.out.println(codeUE + " - " + typeEval + " - " + anneDebut+ " - " + valeurNote);
                                        System.out.println(libelleEval+" "+typeEval+" "+valeurNote+" "+new EstInscritFacade().find(id_estInscrit)+" "+retrouverEvaluation(typeEvaluation));
    ...
    public void importerDiscipline(String pathOut) throws IOException {
        ...
        while (sheetIterator.hasNext()) {
            ...
            Sheet sheet = sheetIterator.next();
            String tmpAnne = dataFormatter.formatCellValue(sheet.getRow(0).getCell(1)).trim();
            String oldMat = dataFormatter.formatCellValue(sheet.getRow(2).getCell(1)).trim();
            String oldLibelle = dataFormatter.formatCellValue(sheet.getRow(2).getCell(1)).trim();
            ...
            while (rowIterator.hasNext()) {
                ...
                if (numrow > 1) {
                    ...
                    Nbheure = Integer.parseInt(dataFormatter.formatCellValue(row.getCell(3)));
                    NbRetard = Integer.parseInt(dataFormatter.formatCellValue(row.getCell(4)));
                    libSemestre = dataFormatter.formatCellValue(row.getCell(5)).toUpperCase().trim();
     ...
     *
     * @param cheminAcces chemin d'acces vers le fichier
     * @throws IOException en cas d'erreur d'ouverture du fichier
     * @throws InvalidFormatException erreur de format
     */
            ...
            ResultSet rs = funcListEns.getResultSet();
                ...
                while(rs.next()){
                    ...
                    if(moyenneUe>=9){
                        ...
                        if(idEns != -1) {
                            //new EstInscritFacade().enregistrer("",enseignement.getUe().getCodeUE(),EstInscrit.Statut.VALIDE, candidat, new EnseignementFacade().find(idEns));
        ...
        while (sheetIterator.hasNext()) {
            ...
            while (rowIterator.hasNext()) {
                ...
                if (numeroLigne == 0) {
                    ...
                    id = (long)row.getCell(1).getNumericCellValue();
                    ...
                    Anonymat anonymat = new Anonymat(libelle, description, num_anonymat, null, evaluation, estInscrit, numeroTable);
    ...
    public void saveNoteAnonymat(String cheminAcces) throws IOException {
        ...
        while(sheetIterator.hasNext()){
            ...
            while (rowIt.hasNext()) {
                ...
                double value;
                NumeroLigne = row.getRowNum();
                String description = dataFormatter.formatCellValue(sheet.getRow(0).getCell(0));
                String libelle = dataFormatter.formatCellValue(sheet.getRow(0).getCell(1));
                if (NumeroLigne >= 5) {
                    try{
                        key = Integer.parseInt(dataFormatter.formatCellValue(row.getCell(0)).trim());
                        value = Double.valueOf(dataFormatter.formatCellValue(row.getCell(1)).trim());
                    ...
                    anonymat = retrouverAnonymatNote(key);

                    note = new Note(libelle, description, value, anonymat.getNumeroTable(), anonymat, anonymat.getEstInscrit(), anonymat.getEvaluation());
            ...
            if (NumeroLigne == 10) {
                ...
                if(etudiant!=null) {
                    for (int i = 5; i < 14; i++) {
                        int h = 0;
                        try {
                            h = Integer.parseInt(dataFormatter.formatCellValue(row.getCell(i)).trim());
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        nb_heure += h;
...
//                System.out.println(dateCompar);
   ...
   /*public ArrayList rangAnnuel(int annee, String classe, String filiere) throws SQLException{
    ...
    public ResultSetMetaData renvoyerChamp(Class entity) {
        return renvoyerChamp(entity.getSimpleName().toLowerCase());
    }
    ...
    public ResultSetMetaData renvoyerChamp(String table) {
        ...
        try {
            ...
            ResultSet resultSet = statement.executeQuery(query.toLowerCase());
