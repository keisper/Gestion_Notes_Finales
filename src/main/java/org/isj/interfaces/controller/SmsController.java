...
public class SmsController implements Initializable {
    ...
    @FXML
    private TableView<Sms> table;
    ObservableList<Sms> listeSms = FXCollections.observableArrayList();

    @FXML
    private TableColumn<Sms, Long> codecolumn;

    @FXML
    private TableColumn<Sms, String> destinatairecolumn;
    ...
    public void listSms() throws SQLException {

        if (Connexion.peutLire(Sms.class)) {

            filtrer(true);

            table.setItems(listeSms);
            ...
            ResultSetMetaData resultSetMetaData = new Isj().renvoyerChamp(Sms.class);
    ...
    Sms smsSelectionne = null;

    public void afficheDetail(Sms sms) {
        ...
        if (Connexion.peutLire(Sms.class)) {
            ...
            if (sms != null) {
                smsSelectionne = sms;
                code.setText(String.valueOf(sms.getCode()));
                destinataire.setText(sms.getDestinataire());
                contenu.setText(sms.getContenu());
                emetteur.setText(sms.getEmetteur());
    ...
    public void handleModifier() {
        ...
        else if (Connexion.peutModifier(Sms.class)) {
            activationDesactivationDetails(gridPane,true);
        }
    }
   ...
   @FXML
    public void handleEnregistrer() {
        ...
        if (Connexion.peutLire(Sms.class) || Connexion.peutModifier(Sms.class)) {
            try {
                String destinaireSms, emetteurSms, contenuSms ;
                destinaireSms = destinataire.getText();
                contenuSms= contenu.getText();
                emetteurSms = emetteur.getText();

                String resultat;
                if (smsSelectionne == null)
                    resultat = smsFacade.enregistrer("","",contenuSms,destinaireSms,emetteurSms);
                else
                    resultat = smsFacade.modifier(smsSelectionne, "", "",contenuSms,destinaireSms,emetteurSms);
    ...
    public void handleSupprimer() {
        ...
        if (Connexion.peutSupprimer(Sms.class)) {
            try {
                ...
                if (smsSelectionne != null) {
                    ...
                    if (result.get() == ButtonType.OK) {
                        String suppression = smsFacade.remove(smsSelectionne);
                        if (suppression != null && suppression.equalsIgnoreCase("succes"))
                            table.getItems().remove(smsSelectionne);
                        else {
    ...
    String requeteFiltrage = "select * from sms";
    ...
    public void handleImprimer() throws IOException {
        ...
        if (Connexion.peutLire(Sms.class)) {
            ...
            selectionChampsReport.setTitre("Liste des sms");
        ...
        private void filtrer(boolean raffraichir) {
            ...
            requeteFiltrage = "select * from sms";
            ...
            listeSms.clear();
            listeSms.addAll(smsFacade.findAllNative(requeteFiltrage));
        }
