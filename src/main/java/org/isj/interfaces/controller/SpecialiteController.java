...
public class SpecialiteController implements Initializable {
    ...
    @FXML
    private TableView<Specialite> table;
    ObservableList<Specialite> listeSpecialite = FXCollections.observableArrayList();

    @FXML
    private TableColumn<Specialite, Long> codecolumn;

    @FXML
    private TableColumn<Specialite, String> libellecolumn;

    @FXML
    private TableColumn<Specialite, Filiere> filierecolumn;
    ...
    public void listSpecialite() throws SQLException {
        ...
        if (Connexion.peutLire(Specialite.class)) {

            filtrer(true);

            table.setItems(listeSpecialite);
            ...
            filierecolumn.setCellValueFactory(cellData -> new SimpleObjectProperty<>(cellData.getValue().getFiliere()));
            codecolumn.setCellValueFactory(cellData -> new SimpleLongProperty(cellData.getValue().getCode()).asObject());

            ResultSetMetaData resultSetMetaData = new Isj().renvoyerChamp(Specialite.class);
    ...
    Specialite specialiteSelectionne = null;

    public void afficheDetail(Specialite specialite) {
        ...
        if (Connexion.peutLire(Specialite.class)) {
            ...
            if (specialite != null) {
                specialiteSelectionne = specialite;
                code.setText(String.valueOf(specialite.getCode()));
                libelle.setText(specialite.getLibelle());
                filiere.setValue(specialite.getFiliere());
            } else {
                specialiteSelectionne = null;
                code.setText("");
                libelle.setText("");
    ...
    public void handleNouveau() {
        ...
        if (Connexion.peutEcrire(Specialite.class)) {
            specialiteSelectionne = null;
    ...
    public void handleModifier() {
        ...
        else if (Connexion.peutModifier(Specialite.class)) {
            activationDesactivationDetails(gridPane,true);
        }
    }
    ...
    public void handleEnregistrer() {
        ...
        if (Connexion.peutLire(Specialite.class) || Connexion.peutModifier(Specialite.class)) {
            try {
                String libelleSpecialite;
                libelleSpecialite = libelle.getText();
                Filiere filiereSpecialite = filiere.getSelectionModel().getSelectedItem();

                String resultat;
                if (specialiteSelectionne == null)
                    resultat = specialiteFacade.enregistrer(libelleSpecialite,"",filiereSpecialite);
                else
                    resultat = specialiteFacade.modifier(specialiteSelectionne, libelleSpecialite,"", filiereSpecialite);
    ...
    public void handleSupprimer() {
        ...
        if (Connexion.peutSupprimer(Specialite.class)) {
            try {
                ...
                if (specialiteSelectionne != null) {
                    ...
                    if (result.get() == ButtonType.OK) {
                        String suppression = specialiteFacade.remove(specialiteSelectionne);
                        if (suppression != null && suppression.equalsIgnoreCase("succes"))
                            table.getItems().remove(specialiteSelectionne);
                        else {
    ...
    public void handleImprimer() throws IOException {
        ...
        if (Connexion.peutLire(Specialite.class)) {
            ...
            selectionChampsReport.setTitre("Liste des spécialités");
    ...
    String requeteFiltrage = "select * from specialite";
    ...
    private void filtrer(boolean raffraichir) {
        ...
        requeteFiltrage = "select * from specialite";
        ...
        listeSpecialite.addAll(specialiteFacade.findAllNative(requeteFiltrage));
    }
