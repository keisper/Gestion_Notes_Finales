...
public class UtilisateurController implements Initializable {
    ...
    @FXML
    private TableColumn<Utilisateur, Long> codecolumn;
    ...
    @FXML
    private TableColumn<Utilisateur, Integer> telcolumn;
    ...
    public void listUtilisateurs() throws SQLException {

        if (Connexion.peutLire(Utilisateur.class)) {

            filtrer(true);

            table.setItems(listeUtilisateurs);
            ...
            datenaissancecolumn.setCellValueFactory(cellData -> new SimpleStringProperty(format.format(cellData.getValue().getDateNaissance())));

            ResultSetMetaData resultSetMetaData = new Isj().renvoyerChamp(Utilisateur.class);
    ...
    public void afficheDetail(Utilisateur utilisateur) {
        ...
        if (Connexion.peutLire(Utilisateur.class)) {
            ...
            if (utilisateur != null) {
                ...
                code.setText(String.valueOf(utilisateur.getCode()));

                nom.setText(utilisateur.getNom());
                prenom.setText(utilisateur.getPrenom());
                if (utilisateur.getSexe().equals(Personne.Sexe.FEMININ)) {
                ...
                datenaissance.setValue(utilisateur.getDateNaissance().toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
                telephone.setText(Integer.toString(utilisateur.getTelephone()));
    ...
    public void handleNouveau() {
        ...
        if (Connexion.peutEcrire(Utilisateur.class)) {
            utilisateurSelectionne = null;
    ...
    public void handleModifier() {
        ...
        else if (Connexion.peutModifier(Utilisateur.class)) {
            activationDesactivationDetails(gridPane,true);
        }
    }
    ...
    public void handleEnregistrer() {
        ...
        if (Connexion.peutLire(Utilisateur.class) || Connexion.peutModifier(Utilisateur.class)) {
            try {
                String nomutilisateur, prenomutilisateur, emailutilisateur, telephoneutilisateur, loginutilisateur;
                nomutilisateur = nom.getText();
                prenomutilisateur = prenom.getText();
                telephoneutilisateur = telephone.getText();
                emailutilisateur = email.getText();
                loginutilisateur = login.getText();
                Personne.Sexe sexe = masculin.isSelected() ? Personne.Sexe.MASCULIN : Personne.Sexe.FEMININ;
                Date dateNaissanceutilisateur = Date.from(datenaissance.getValue().atStartOfDay(ZoneId.systemDefault()).toInstant());
                //Haschage du mot de passe
                String passwordUtilisateur = new Connexion().hachage(password.getText().toString());

                String resultat;
                if (utilisateurSelectionne == null)
                    resultat = utilisateurFacade.enregistrer("","",nomutilisateur,prenomutilisateur,emailutilisateur,Integer.parseInt(telephoneutilisateur),dateNaissanceutilisateur,sexe, Personne.Statut.ACTIVE, loginutilisateur, passwordUtilisateur);
                else
                    resultat = utilisateurFacade.modifier(utilisateurSelectionne,"","",nomutilisateur,prenomutilisateur,emailutilisateur,Integer.parseInt(telephoneutilisateur),dateNaissanceutilisateur,sexe, Personne.Statut.ACTIVE, loginutilisateur, passwordUtilisateur);
    ...
    public void handleSupprimer() {
        ...
        if (Connexion.peutSupprimer(Utilisateur.class)) {
            try {
                ...
                if (utilisateurSelectionne != null) {
                    ...
                    if (result.get() == ButtonType.OK) {
                        String suppression = utilisateurFacade.remove(utilisateurSelectionne);
                        if (suppression != null && suppression.equalsIgnoreCase("succes"))
                            table.getItems().remove(utilisateurSelectionne);
                        else {
    ...
    public void handleImprimer() throws IOException {
        ...
        if (Connexion.peutLire(Utilisateur.class)) {
            ...
            selectionChampsReport.setTitre("Liste des utilisateurs");
    ...
    String requeteFiltrage = "select * from utilisateur";
    ...
    private void filtrer(boolean raffraichir) {
        ...
        requeteFiltrage = "select * from utilisateur";
        ...
        listeUtilisateurs.addAll(utilisateurFacade.findAllNative(requeteFiltrage));
    }
